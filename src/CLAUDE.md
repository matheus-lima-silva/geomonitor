...
`AppIcon` mantem um `ICON_MAP` com **aliases semanticos** (`plus`, `save`, `edit`, `trash`, `close`, `check`, `alert`, `dashboard-nav`, `projects-nav`, `licenses-nav`, `inspections-nav`, `erosions-nav`, `visit-nav`, `route-plan`, `hash`, `folder-input`, `radio-tower`, `file-plus-2`, `cloud-check`, `map-pin`, `check-circle-2` etc.). Esses aliases padronizam o visual mesmo que o icone lucide subjacente mude.
