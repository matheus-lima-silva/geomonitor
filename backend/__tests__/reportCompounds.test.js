...
describe('Report Compounds API Integration Tests (Mocked DB)', () => {
    beforeEach(async () => {
        ...
        await request(app)
            .post('/api/report-workspaces')
            .set(AUTH_HEADER)
            .send({ data: { id: 'RW-C2', nome: 'Workspace B', projectId: 'PRJ-01' } });
    });

    it('cria composto, adiciona workspaces, reordena blocos, roda preflight e enfileira geracao', async () => {
        ...
        expect(addWorkspaceResponse.body.data.orderJson).toEqual(['RW-C1']);

        const addSecondWorkspaceResponse = await request(app)
            .post(`/api/report-compounds/${compoundId}/add-workspace`)
            .set(AUTH_HEADER)
            .send({ data: { workspaceId: 'RW-C2' } });

        expect(addSecondWorkspaceResponse.status).toBe(200);
        expect(addSecondWorkspaceResponse.body.data.workspaceIds).toEqual(['RW-C1', 'RW-C2']);
        expect(addSecondWorkspaceResponse.body.data.orderJson).toEqual(['RW-C1', 'RW-C2']);

        const reorderResponse = await request(app)
            .post(`/api/report-compounds/${compoundId}/reorder`)
            .set(AUTH_HEADER)
            .send({ data: { orderJson: ['RW-C2', 'RW-C1'] } });

        expect(reorderResponse.status).toBe(200);
        expect(reorderResponse.body.data.orderJson).toEqual(['RW-C2', 'RW-C1']);
        ...
        expect(preflightResponse.body.data.workspaceCount).toBe(2);
