...
async function list() {
    ...
    return result.rows.map((row) => hydrateMediaRow({
        ...row,
        payload: {
            ...(row.payload || {}),
            purpose: row.purpose,
...
async function remove(id) {
    const normalizedId = normalizeText(id);
    if (!isPostgresBackend()) {
        return require('../data').getDataStore().deleteDoc('mediaAssets', normalizedId);
    }
