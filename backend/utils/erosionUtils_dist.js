...
// utils/erosionUtils.js
var erosionUtils_exports = {};
__export(erosionUtils_exports, {
  EROSION_LOCATION_OPTIONS: () => EROSION_LOCATION_OPTIONS,
  EROSION_REMOVED_FIELDS: () => EROSION_REMOVED_FIELDS,
  EROSION_TECHNICAL_ENUMS: () => EROSION_TECHNICAL_ENUMS,
  EROSION_TECHNICAL_OPTIONS: () => EROSION_TECHNICAL_OPTIONS,
  LOCAL_CONTEXTO_DEFAULT: () => LOCAL_CONTEXTO_DEFAULT,
  LOCAL_CONTEXTO_TIPO_OPTIONS: () => LOCAL_CONTEXTO_TIPO_OPTIONS,
  appendFollowupEvent: () => appendFollowupEvent,
  buildCriticalityInputFromErosion: () => buildCriticalityInputFromErosion,
  buildErosionReportRows: () => buildErosionReportRows,
  buildErosionsCsv: () => buildErosionsCsv,
  ...
  deriveCriticalityDimensionClasses: () => deriveCriticalityDimensionClasses,
  deriveErosionTypeFromTechnicalFields: () => deriveErosionTypeFromTechnicalFields,
  filterErosionsForReport: () => filterErosionsForReport,
  getLocalContextLabel: () => getLocalContextLabel,
  isHistoricalErosionRecord: () => isHistoricalErosionRecord,
  normalizeEnumArray: () => normalizeEnumArray,
  normalizeErosionTechnicalFields: () => normalizeErosionTechnicalFields,
  normalizeFollowupEventType: () => normalizeFollowupEventType,
  normalizeFollowupHistory: () => normalizeFollowupHistory,
  normalizeLocalContexto: () => normalizeLocalContexto,
  stripRemovedErosionFields: () => stripRemovedErosionFields,
  toIsoDate: () => toIsoDate,
  validateErosionLocation: () => validateErosionLocation,
  validateErosionTechnicalFields: () => validateErosionTechnicalFields
});
...
// utils/erosionUtils.js
var LOCAL_CONTEXTO_TIPO_OPTIONS = [
...
var EROSION_LOCATION_OPTIONS = LOCAL_CONTEXTO_TIPO_OPTIONS.map((item) => item.label);
var LOCAL_CONTEXTO_LABEL_BY_TIPO = LOCAL_CONTEXTO_TIPO_OPTIONS.reduce((acc, item) => {
  acc[item.value] = item.label;
  return acc;
}, {});
var EROSION_LOCATION_ALIASES = {
  "na faixa de servidao": "faixa_servidao",
  "na faixa de servid\xE3o": "faixa_servidao",
...
function getLocalContextLabel(localTipo) {
...
function normalizeLocalContexto(data = {}) {
  const source = data && typeof data === "object" ? data : {};
  const incoming = source.localContexto && typeof source.localContexto === "object" ? source.localContexto : {};
...
function normalizeErosionTechnicalFields(data = {}) {
...
function deriveCriticalityDimensionClasses(data = {}) {
...
function validateErosionLocation(data) {
...
function validateErosionTechnicalFields(data = {}) {
  const raw = data && typeof data === "object" ? data : {};
  const removed = EROSION_REMOVED_FIELDS.filter((field) => hasValue(raw[field]));
  if (removed.length > 0) {
    return {
      ok: false,
      message: `Campos legados removidos no schema can\xF4nico: ${removed.join(", ")}.`,
  ...
  for (let i = 0; i < multiRules.length; i += 1) {
    ...
    const invalid = raw[field].map((item) => normalizeText(item).toLowerCase()).find((item) => item && !allowed.includes(item));
    if (invalid) {
      return {
        ok: false,
  ...
  for (let i = 0; i < numericRules.length; i += 1) {
    ...
    if (sourceValue === null || sourceValue === void 0 || String(sourceValue).trim() === "") continue;
...
function summarizeEvent(change, previous) {
  if (change.origem === "cadastro" && change.registroHistoricoNovo) {
    return change.intervencaoRealizadaNova ? `Cadastro historico da erosao. Intervencao ja realizada: ${change.intervencaoRealizadaNova}` : "Cadastro historico da erosao com intervencao ja realizada.";
  }
  if (change.origem === "cadastro") return "Cadastro inicial da erosao.";
  if (change.origem === "vistoria" && change.registroHistoricoNovo) {
    return change.intervencaoRealizadaNova ? `Erosao historica registrada durante vistoria. Intervencao ja realizada: ${change.intervencaoRealizadaNova}` : "Erosao historica registrada durante vistoria.";
  }
  ...
  if (change.registroHistoricoAnterior !== change.registroHistoricoNovo) {
    pieces.push(change.registroHistoricoNovo ? "registro marcado como historico de acompanhamento" : "registro voltou ao modo tecnico");
  }
  ...
  if (change.registroHistoricoNovo && String(change.intervencaoRealizadaAnterior || "") !== String(change.intervencaoRealizadaNova || "")) {
...
function buildFollowupEvent(previous, next, meta = {}) {
  ...
  const event = {
    ...
    origem,
    vistoriaId: normalizeText(next?.vistoriaId) || void 0,
    statusAnterior: previous ? previousStatus : void 0,
  ...
  const hasRelevantChange = origem === "cadastro" || origem === "vistoria" || event.registroHistoricoAnterior !== event.registroHistoricoNovo || event.statusAnterior !== event.statusNovo || String(event.torreAnterior || "") !== String(event.torreNova || "") || String(event.localTipoAnterior || "") !== String(event.localTipoNovo || "") || String(event.intervencaoRealizadaAnterior || "") !== String(event.intervencaoRealizadaNova || "") || String(event.obsAnterior || "") !== String(event.obsNovo || "");
...
function normalizeFollowupEventType(item) {
...
function getErosionInspectionIds(erosion) {
  ...
  const merged = [primary, ...extra.map((item) => normalizeText(item))].filter(Boolean);
  return [...new Set(merged)];
}
function resolveErosionProjectId(erosion, inspectionsById = /* @__PURE__ */ new Map()) {
...
function resolveErosionDate(erosion, inspectionsById = /* @__PURE__ */ new Map()) {
  const candidates = [
...
function filterErosionsForReport(erosions, { projetoId, anos }, inspections = []) {
  const selectedYearsRaw = Array.isArray(anos) ? anos : [];
  const selectedYears = new Set(
    selectedYearsRaw.map((year) => Number(year)).filter((year) => Number.isInteger(year) && year >= 1900 && year <= 9999)
  );
...
function buildErosionReportRows(erosions) {
  return (erosions || []).map((item) => {
    ...
    return {
      ...
      dimensionamento: technical.dimensionamento,
      criticidadeCodigo: item?.criticalidade?.codigo || item?.criticalidadeV2?.codigo || "",
      criticidadeClasse: item?.criticalidade?.criticidade_classe || item?.criticalidadeV2?.criticidade_classe || "",
      criticidadeScore: item?.criticalidade?.criticidade_score ?? item?.criticalidadeV2?.criticidade_score ?? "",
...
function buildErosionsCsv(rows) {
  const headers = [
    "id",
    "projetoId",
    "vistoriaId",
    "torreRef",
    "localContexto.localTipo",
    "localContexto.localTipoLabel",
    "localContexto.localDescricao",
    "localContexto.exposicao",
    "localContexto.estruturaProxima",
    "tipo",
    "estagio",
    "status",
    "impacto",
