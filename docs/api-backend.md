...
| GET | `/api/paec/plants/:id` | `requireActiveUser` | Ficha completa (`fields{chave:valor}` + `listItems{listKey:[{colKey:valor}]}` + `sectionFlags{sectionKey:{enabled,titleOverride}}`) + `pendencies[]` + `stats` |
