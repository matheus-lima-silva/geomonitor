...
Modulo do portal relat.lima.rio.br (Plano de Atendimento as Emergencias da Central). Ficha "titulo chave -> texto valor" por usina, **unica e compartilhada entre editores** (nao user-owned). Modelo relacional (migration `0026`-`0030`): `paec_templates` (revisoes do modelo canonico tokenizado — docx `{{placeholders}}` no storage de midia + `manifest` JSONB gerado por `worker/tools/paec_tokenizer.py`; no maximo 1 `active` por `name`) + `paec_plants` (header por usina com `version` para concorrencia otimista, mais `plant_type`/`installed_capacity_mw` — identidade basica da usina, fora do manifest tokenizado, nao entra no DOCX) + `paec_plant_fields` (uma linha por campo preenchido, auditavel) + `paec_plant_list_items` (Fase 2 — uma linha por item de bloco tabular: brigadistas, recursos BEM, extintores, pontos de encontro, contatos internos/externos, plantoes; `row` JSONB porque as colunas variam por `manifest.blocks[].columns`; replace-on-save integral por `list_key`, sem auditoria por celula) + `paec_plant_section_flags` (Fase 3 — secoes `12.1.x` liga/desliga por usina; so guarda linha pra secao que desvia do padrao — desligada ou com `title_override`; ausente = ligada com o titulo padrao do manifest) + `paec_plant_assets` (Fase 4 — anexos com imagem por usina: rota de fuga e unifilar, uma linha por imagem de um `manifest.imageSlots[].assetKey` na ordem de insercao; `media_asset_id` com **FK real `ON DELETE RESTRICT`** pra `media_assets`, mesmo padrao de `paec_templates.tokenized_docx_media_id`; upload das imagens pelo fluxo generico `/api/media/upload-url` com `purpose: 'paec_attachment'`). Body validado por `backend/schemas/paecSchemas.js` (envelope `{ data, meta }`). Pendencias computadas server-side por `backend/utils/paecPendencies.js` (fonte unica: rota de detalhe, contexto do worker e `resultMeta` do job) — bloco `kind=list` so vira pendencia se `paec_plant_list_items` nao tiver nenhuma linha para aquele `list_key`; imageSlot so vira pendencia (`kind=image`) se `paec_plant_assets` nao tiver nenhuma imagem para aquele `asset_key` (`maxImages` do manifest e teto de UI, nao minimo); `kind=manual` e sempre pendencia fixa (ainda sem edicao estruturada).
...
`data` do plant: `{ name, projectId?, plantType?, installedCapacityMw?, version?, copyFromId?, fields{ <chave do manifest>: <valor> }, listItems{ <listKey do manifest>: [ {<colKey>: <valor>}, ... ] }, sectionFlags{ <sectionKey do manifest>: {enabled?, titleOverride?} }, assets{ <assetKey do manifest>: [<mediaAssetId>, ...] } }`. `plantType` in `UHE|PCH|CGH|Subestacao`. `copyFromId` copia tambem `listItems`/`sectionFlags`/`assets` da usina de origem. `assets` e substituido por inteiro a cada save (mesma semantica de `listItems`). `listItems` e substituido por inteiro a cada save (sem merge por linha — o array manda); `sectionFlags` so precisa de entrada pra secao que desvia do padrao (`enabled: false` ou `titleOverride` setado — omitir a chave = ligada, titulo padrao). O DOCX e renderizado pelo worker (`worker/paec_renderer.py`, `kind=paec_report`): baixa o template tokenizado, substitui `{{chave}}`/`{{chave|upper|title}}`, campo sem valor vira `[[PENDENTE: <label>]]` com realce amarelo, e devolve `resultMeta{pendencies,stats}` no `PUT /report-jobs/:id/complete` (persistido no payload do job; exposto no `GET /report-jobs/:id`). Contexto: `buildPaecContext` em `backend/utils/reportJobContext.js` (inclui `listItems`/`sectionFlags`/`sections`/`imageSlots`/`assets` no `renderModel.paecReport`). Blocos `kind=list` sao renderizados clonando a linha-modelo da tabela uma vez por item de `listItems`, preenchendo as celulas por `columns[].key`; sem item nenhum, uma linha `[[PENDENTE: <label>]]` substitui a linha-exemplo do modelo — nunca sobra dado de outra usina no documento gerado. Localizacao da tabela: pela ancora `manifest.blocks[].anchorContext` quando o span ainda existe no template (extintores, brigadistas, recursos materiais BEM, pontos de encontro), ou por `manifest.blocks[].headerMatch` (texto do cabecalho da tabela) quando nao ha span nenhum marcado no documento original (contatos internos/externos, plantoes). Secoes `12.1.x` desligadas via `sectionFlags`: o renderer localiza cada heading pelo texto (`manifest.sections[].defaultTitle`, ainda realcado no template — `section_title` nunca e tokenizado no `apply`), remove o conteudo do heading ate o proximo heading do documento, renumera sequencialmente as remanescentes de cada `renumberGroup` (aplicando `titleOverride` quando presente) e injeta `<w:updateFields w:val="true"/>` em settings.xml pro Word oferecer atualizar o sumario ao abrir. Anexos com imagem (`manifest.imageSlots[]`, Fase 4): o renderer localiza a ancora do slot (span `kind=image` realcado), REMOVE as imagens-exemplo do modelo na regiao do anexo (ate o heading do proximo anexo; legendas/notas de texto sao preservadas) e insere uma imagem por `mediaAssetId` de `assets[assetKey]`, centralizada e dimensionada pela mesma regra dos demais renderers (largura 15cm, teto de altura); slot vazio vira paragrafo `[[PENDENTE: <label>]]` realcado — nunca sobra diagrama de outra usina no documento gerado. As midias sao prefetchadas em paralelo pelo `worker/job_processor.py` (caso `paec_report` em `_collect_media_asset_ids`). So cobre as secoes que ja tem span `kind=section_title` curado no `mapping.yaml` (17 das ~27 subsecoes de "recursos internos" do REV 10 — as demais nunca foram realcadas no documento original, mesmo gap dos blocos `contatos_internos`/`contatos_externos`/`plantoes` antes de virarem `headerMatch`).

Seed das usinas reais (Fase 6): `backend/scripts/seedPaecPlant.js --seed backend/scripts/seed/<usina>.json` cria/atualiza a ficha via API (autentica por `/api/auth/login` com `GEOMONITOR_EMAIL`/`GEOMONITOR_PASSWORD` — as rotas de plants exigem JWT de editor, nao aceitam `x-worker-token`). Os seeds `paec_pch_anta.json` e `paec_uhe_simplicio.json` foram extraidos dos PAECs antigos Furnas 2021; valores legados entram como ponto de partida e os campos organizacionais AXIA ficam vazios de proposito (viram pendencia residual na ficha — dados que so o usuario/campo tem: gerencias, siglas e responsaveis atuais).
...
- `includeCriticidadeTable`: `false` (default) — quando `true`, insere a Tabela 1 (Grau de criticidade) na secao "Descricao das Atividades".
