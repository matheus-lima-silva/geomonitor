...
Modulo do portal relat.lima.rio.br (Plano de Atendimento as Emergencias da Central). Ficha "titulo chave -> texto valor" por usina, **unica e compartilhada entre editores** (nao user-owned). Modelo relacional (migration `0026`, `0027`, `0028`): `paec_templates` (revisoes do modelo canonico tokenizado — docx `{{placeholders}}` no storage de midia + `manifest` JSONB gerado por `worker/tools/paec_tokenizer.py`; no maximo 1 `active` por `name`) + `paec_plants` (header por usina com `version` para concorrencia otimista, mais `plant_type`/`installed_capacity_mw` — identidade basica da usina, fora do manifest tokenizado, nao entra no DOCX) + `paec_plant_fields` (uma linha por campo preenchido, auditavel) + `paec_plant_list_items` (Fase 2 — uma linha por item de bloco tabular: brigadistas, recursos BEM, extintores, pontos de encontro; `row` JSONB porque as colunas variam por `manifest.blocks[].columns`; replace-on-save integral por `list_key`, sem auditoria por celula). Body validado por `backend/schemas/paecSchemas.js` (envelope `{ data, meta }`). Pendencias computadas server-side por `backend/utils/paecPendencies.js` (fonte unica: rota de detalhe, contexto do worker e `resultMeta` do job) — bloco `kind=list` so vira pendencia se `paec_plant_list_items` nao tiver nenhuma linha para aquele `list_key`; `kind=manual` e sempre pendencia fixa (ainda sem edicao estruturada).
...
| GET | `/api/paec/plants/:id` | `requireActiveUser` | Ficha completa (`fields{chave:valor}` + `listItems{listKey:[{colKey:valor}]}`) + `pendencies[]` + `stats` |
...
`data` do plant: `{ name, projectId?, plantType?, installedCapacityMw?, version?, copyFromId?, fields{ <chave do manifest>: <valor> }, listItems{ <listKey do manifest>: [ {<colKey>: <valor>}, ... ] } }`. `plantType` in `UHE|PCH|CGH|Subestacao`. `copyFromId` copia tambem os `listItems` da usina de origem. `listItems` e substituido por inteiro a cada save (sem merge por linha — o array manda). O DOCX e renderizado pelo worker (`worker/paec_renderer.py`, `kind=paec_report`): baixa o template tokenizado, substitui `{{chave}}`/`{{chave|upper|title}}`, campo sem valor vira `[[PENDENTE: <label>]]` com realce amarelo, e devolve `resultMeta{pendencies,stats}` no `PUT /report-jobs/:id/complete` (persistido no payload do job; exposto no `GET /report-jobs/:id`). Contexto: `buildPaecContext` em `backend/utils/reportJobContext.js` (inclui `listItems` no `renderModel.paecReport`). Blocos `kind=list` sao renderizados clonando a linha-modelo da tabela (localizada pela ancora `manifest.blocks[].anchorContext` no proprio DOCX) uma vez por item de `listItems`, preenchendo as celulas por `columns[].key`; sem item nenhum, uma linha `[[PENDENTE: <label>]]` substitui a linha-exemplo do modelo — nunca sobra dado de outra usina no documento gerado. So funciona pros blocos ja com `columns` curadas em `mapping.yaml` (extintores, brigadistas, recursos materiais BEM, pontos de encontro); blocos `kind=manual` (contatos internos/externos, plantoes) continuam so como pendencia fixa nesta fase.
