...
- **`PhotoPreviewModal`** (`src/features/reports/components/PhotoPreviewModal.jsx`) — reescrito sobre o `Modal` (size `2xl`) em 3 faixas: header com tile de icone + ID mono + toolbar (`download`, `trash-2` com confirmacao inline `dangerGhost`, fechar); palco escuro (`bg-slate-950`) limpo (sem overlays); painel lateral 340px com card de metadados, acao `.docx` (estado verde quando incluida), legenda com **autosave** (sem botao Salvar: debounce 650ms -> "Salvando..." -> "Salvo automaticamente"), mini-mapa e acao "Criar erosao". **Navegacao e contador ficam no rodape** (barra central de setas `IconButton outline` + `N / TOTAL`), nao sobre a foto — decisao final do handoff (`data-nav/counter="footer"`). Legenda usa campo **preenchido** (`Textarea variant="filled"`) e os rotulos sao **sentence-case** ("Legenda da foto", "Localizacao da foto"), nao micro-label maiusculo. Atalhos: setas navegam, `E` inclui, `Del` lixeira.
...
- **`Textarea`** ganhou `variant` `default` | `filled` — `filled` = fundo `app-surfaceMuted` + borda transparente, volta a branco no foco (estilo "preenchido" do handoff). Aditivo (default mantem o comportamento atual).
